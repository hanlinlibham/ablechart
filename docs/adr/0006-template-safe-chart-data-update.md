...
上层产品定位已经从"LLM 生成 PPT"收敛为"专业数据驱动的可编辑 PowerPoint 更新与生成平台"。这对 `chartengine` 的含义很具体:
...
`chartengine` 将把 **template-safe chart data update** 作为一等底层能力,但它只解决 chart 技术层问题,不接管产品层模板资产管理。
...
`chartengine` 输出 technical inventory,不输出业务 manifest。
...
这些断言并入 ADR-0002 的 L2/L3 质量闸门。涉及已有模板更新的 golden 应加入 pptfi `goldens/` 体系,但底层断言放在 `chartengine` 测试中。
